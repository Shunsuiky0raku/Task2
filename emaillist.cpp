...
int EmailList::determinePriority(const std::string& text) {
    ...
    for (int i = 0; i < wordCount; i++) {
        if (highPriorityWords.contains(text)) return 1; // High Priority
        if (mediumPriorityWords.contains(text)) return 2; // Medium Priority
        if (lowPriorityWords.contains(text)) return 3; // Low Priority
    }
...
void EmailList::insertEmail(int id, const std::string& sender, const std::string& subject,
                            const std::string& body, int priority, const std::string& date) {
    Email email(id, sender, "", subject, body, date); // Adjust constructor arguments
    if (priority == 1) {
        highPriority.push(email);
    } else if (priority == 2) {
        mediumPriority.push(email);
    } else {
        lowPriority.push(email);
    }
}
...
void EmailList::loadEmailsFromCSV(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        else if (priority == 3) lowPriority.push(email);
    }

    std::cout << "Emails prioritized successfully from CSV file.\n";
}
...
void EmailList::displayList(const std::string& title, Email* head) const {
    std::cout << "\n" << title << ":\n";
    while (head != nullptr) {
        std::cout << "ID: " << head->id << "\n"
                  << "From: " << head->sender << "\n"
